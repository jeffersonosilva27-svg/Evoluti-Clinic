...
export default function Agenda({ selectedClinic }: { selectedClinic: string | null }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      const found = snap.docs.map(doc => ({ uid: doc.id, ...doc.data() } as UserProfile));
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      const docs = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Appointment));
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-6">
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Visão Geral da Semana</p>
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Filtrar Equipe</span>
          </div>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1200px]">
            ...
            <div className="grid grid-cols-[200px_repeat(7,1fr)] bg-slate-50 border-b border-slate-200">
              <div className="p-4 border-r border-slate-200 text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center justify-center">
                Profissional
              </div>
              {weekDays.map((day, i) => (
                <div 
                  key={i} 
                  className={`p-4 flex flex-col items-center justify-center border-r border-slate-200 last:border-r-0 ${
                ...
                >
                  ...
                  <span className={`text-base font-black ${isSameDay(day, new Date()) ? 'text-brand-primary' : 'text-slate-800'}`}>
                    {format(day, 'dd/MM')}
                  </span>
                </div>
              ))}
            ...
            <div className="divide-y divide-slate-100">
              {professionals.map((pro) => (
                <div key={pro.uid} className="grid grid-cols-[200px_repeat(7,1fr)] min-h-[160px] group">
                  {/* Pro Info Column */}
                  <div className={`p-6 border-r border-slate-200 flex flex-col items-center justify-center text-center gap-3 transition-colors ${
                    pro.uid === profile?.uid ? 'bg-blue-50/20' : 'bg-white group-hover:bg-slate-50'
                  }`}>
                    <div className="w-12 h-12 rounded-full bg-white border-2 border-slate-100 flex items-center justify-center text-slate-400 font-black shadow-sm overflow-hidden">
                      {pro.uid === profile?.uid ? (
                        <div className="w-full h-full bg-brand-primary text-white flex items-center justify-center text-sm">VOCÊ</div>
                      ) : pro.name.charAt(0)}
                    </div>
                    <div>
                      <p className="text-xs font-black text-slate-800 leading-tight">{pro.name}</p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-1">{pro.role.replace('_', ' ')}</p>
                    </div>
                  </div>
                    ...
                    >
                      {appointments
                        .filter(app => {
                          ...
                          return isSameDay(appDate, day) && app.professionalId === pro.uid;
                        })
